...
st.header("EFG Asset Management - Portfolio Construction Tool - Alpha - 20240314 15:43", divider=True)
...
with structure:
    ...
    if len(bond_selection) > 0:
        ...
        bond_df = pd.DataFrame([['EuroDollars USD',0]],columns=['Asset','Weight'])
    ...
    if (
                ((bond > 0) and (len(bond_selection) == 0)) or 
                ((equity > 0) and (len(equity_selection) == 0)) or 
                ((alternative > 0) and (len(alternative_selection) == 0))):
         selection_error = True
         st.write("Error in selection")
    else:
